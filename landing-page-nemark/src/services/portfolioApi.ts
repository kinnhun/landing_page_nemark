...
export const getPortfolioSettings = async (): Promise<PortfolioSettings> => {
  try {
    const res = await fetch(`${API_URL}/settings/portfolio`, {
      cache: 'no-store',
      headers: {
        'Pragma': 'no-cache',
    ...
    const json = (await res.json()) as ApiResponse<PortfolioSettings>;
    return json.data;
  } catch (err) {
    console.warn('portfolioApi.getPortfolioSettings: failed to fetch from backend, using default portfolio', err);
    return {
...
export const savePortfolioSettings = async (settings: PortfolioSettings): Promise<PortfolioSettings | null> => {
  try {
    const res = await fetch(`${API_URL}/settings/portfolio`, {
      method: 'POST',
